...
async fn main() {
    ...
    if std::env::args().nth(1) == Some("server".to_string()) {
        ...
        service
            .listen("/ip4/0.0.0.0/tcp/2233".parse().expect("listen address"))
            .await
            .expect("listen");
        ...
        let server = {
            ...
            format!("/ip4/{}/tcp/2233", server)
                .parse()
                .expect("server address")
        };

        service
            .dial(server, TargetProtocol::All)
            .await
            .expect("dial");
